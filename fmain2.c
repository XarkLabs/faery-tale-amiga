...
void dohit(int32_t i, int32_t j, int32_t fc, int16_t wt)
{
    ...
    if (anim_list[j].race == 0x8a || anim_list[j].race == 0x8b)
        return;
    ...
    if (i == -1)
        ...
        effect(5, 3200 + bitrand(511));
    else if (j == 0)
        effect(0, 800 + bitrand(511));
    else
        effect(3, 400 + rand256());
    /* push back the foe!! Yar! */
    if (anim_list[j].type != DRAGON && anim_list[j].type != SETFIG && move_figure(j, fc, 2) &&
        (i >= 0))
        move_figure(i, fc, 2);
    checkdead(j, 5);
}

extern int16_t nearest;
extern int16_t xtype;
char           turtle_eggs;
...
void aftermath(void)
{
    ...
    for (i = 3; i < anix; i++)
    {
        if (anim_list[i].type != ENEMY)
            ;
        else if (anim_list[i].state == DEAD)
            dead++;
        else if (anim_list[i].goal == FLEE)
            flee++;
    }
    if (anim_list[0].vitality < 1)
        ; /* if dead, no message */
    else if (anim_list[0].vitality < 5 && dead)
        print("Bravely done!");
    else if (xtype < 50)
    {
...
int nearest_fig(char constraint, int16_t dist)
{
    ...
    for (i = 1; i < anix2; i++)
    {
        if (anim_list[i].type == OBJECTS && (constraint || anim_list[i].index == 0x1d))
            continue;
        d = calc_dist(i, 0);
        if (d < dist)
        {
...
int calc_dist(int32_t a, int32_t b)
{
    ...
    if (x < 0)
        x = -x;
    y = anim_list[a].abs_y - anim_list[b].abs_y;
    if (y < 0)
        y = -y;
    if (x > (y + y))
        return x;
...
USHORT pagecolors[] = {0x0000, 0x0FFF, 0x0E96, 0x0B63, 0x0631, 0x07BF, 0x0333, 0x0DB8,
...
void fade_page(int16_t r, int16_t g, int16_t b, int16_t limit, USHORT * colors)
{
    ...
    {
        if (secret_timer)
            pagecolors[31] = 0x00f0;
        else
            pagecolors[31] = 0x0445;
    }
    else
        pagecolors[31] = 0x0bdf;
...
void fade_down(void)
{
    ...
    for (i = 100; i >= 0; i -= 5)
    {
        fade_page(i, i, i, FALSE, pagecolors);
        Delay(1);
    }
}
...
extern uint8_t *shape_mem, *nextshape;
extern uint16_t safe_r;
...
extern uint8_t *(track[32]), *scoremem;
...
{
    int16_t          i, scol, h;
    register int32_t bcol, dcol, rate, wide;
    struct BitMap *  temp;
    for (i = 0; i < 22; i++)
    {
        ...
        if (i < 11)
        {
            ...
            for (scol = wide; scol < 136; scol += rate)
            {
                h = page_det(scol);
                BltBitMap(
                    &pagea, bcol + scol, h, temp, 161 + dcol, h, wide, 200 - h - h, 0xC0, 0x1f, 0);
                dcol += wide;
            }
        ...
        else
        {
            ...
            for (scol = wide; scol < 136; scol += rate)
            {
                ...
                dcol += wide;
                BltBitMap(
                    &pageb, bcol - scol, h, temp, bcol - dcol, h, wide, 200 - h - h, 0xC0, 0x1f, 0);
            }
            scol = 135;
            h    = 7;
...
UBYTE fallstates[] = {0,    0,    0,    0,    0,    0,    0x20, 0x22, 0x3a, 0x6f, 0x70, 0x71,
                      0x24, 0x27, 0x3c, 0x6f, 0x70, 0x71, 0x37, 0x38, 0x3d, 0x6f, 0x70, 0x71};

char bow_x[32] = {1,  2,  3,  4,  3,  2,  1,  0,  3, 2, 0, -2, -3, -2, 0,  2,
                  -3, -3, -3, -3, -3, -3, -3, -2, 0, 1, 1, 1,  0,  -2, -3, -2};
char bow_y[32] = {8, 8, 8, 7, 8, 8, 8, 8, 11, 12, 13, 13, 13, 13, 13, 12,
...
BYTE witchpoints[] = {
    /* 64 points - 256 entries */
    0,   100, 0,   10,   9,   99,  0,   9,    19,   98,  1,   9,   29,   95,  2,   9,   38,   92,
    3,   9,   47,  88,   4,   8,   55,  83,   5,    8,   63,  77,  6,    7,   70,  70,  7,    7,
    77,  63,  7,   6,    83,  55,  8,   5,    88,   47,  8,   4,   92,   38,  9,   3,   95,   29,
    9,   2,   98,  19,   9,   1,   99,  9,    9,    0,   100, 0,   10,   0,   99,  -10, 9,    -1,
    98,  -20, 9,   -2,   95,  -30, 9,   -3,   92,   -39, 9,   -4,  88,   -48, 8,   -5,  83,   -56,
    8,   -6,  77,  -64,  7,   -7,  70,  -71,  7,    -8,  63,  -78, 6,    -8,  55,  -84, 5,    -9,
    47,  -89, 4,   -9,   38,  -93, 3,   -10,  29,   -96, 2,   -10, 19,   -99, 1,   -10, 9,    -100,
    0,   -10, 0,   -100, 0,   -10, -10, -100, -1,   -10, -20, -99, -2,   -10, -30, -96, -3,   -10,
    -39, -93, -4,  -10,  -48, -89, -5,  -9,   -56,  -84, -6,  -9,  -64,  -78, -7,  -8,  -71,  -71,
    -8,  -8,  -78, -64,  -8,  -7,  -84, -56,  -9,   -6,  -89, -48, -9,   -5,  -93, -39, -10,  -4,
    -96, -30, -10, -3,   -99, -20, -10, -2,   -100, -10, -10, -1,  -100, -1,  -10, -1,  -100, 9,
    -10, 0,   -99, 19,   -10, 1,   -96, 29,   -10,  2,   -93, 38,  -10,  3,   -89, 47,  -9,   4,
    -84, 55,  -9,  5,    -78, 63,  -8,  6,    -71,  70,  -8,  7,   -64,  77,  -7,  7,   -56,  83,
    -6,  8,   -48, 88,   -5,  8,   -39, 92,   -4,   9,   -30, 95,  -3,   9,   -20, 98,  -2,   9,
    -10, 99,  -1,  9,
};
...
UBYTE itrans[] = {
    QUIVER,  35, B_STONE,  9,  G_JEWEL,   10, VIAL, 11, C_ORB,    12, B_TOTEM,   13, G_RING,   14,
    J_SKULL, 15, M_WAND,   4,  27,        5,  8,    2,  9,        1,  12,        0,  10,       3,
    ROSE,    23, FRUIT,    24, STATUE,    25, BOOK, 26, SHELL,    6,  155,       7,  136,      27,
    137,     28, 138,      29, 139,       22, 140,  30, GOLD_KEY, 16, GREEN_KEY, 17, BLUE_KEY, 18,
    RED_KEY, 19, GREY_KEY, 20, WHITE_KEY, 21, 0,    0};

UBYTE rand_treasure[] = {SACKS,
                         SACKS,
                         SACKS,
                         SACKS,
                         CHEST,
...
struct object /* 250 objects, for a start */
    ...
    ob_list2[] =
        {                      /* swamp land objects */
         ...
         TENBLANKS},
    ob_list3[] =
        {/* maze forest (south) and manor objects (and tambry) */
         {19298, 16128, CHEST, 1},
    ...
    ob_list6[] =
        {                       /* lava plain objects */
         ...
         TENBLANKS},
    ob_list7[] =
        {                          /* southern mountain land objects */
...
struct object * ob_table[10] = {ob_list0,
                                ob_list1,
                                ob_list2,
                                ob_list3,
                                ob_list4,
...
int16_t mapobs[10] = {3, 1, 5, 12, 3, 5, 1, 1, 61 + 16, 9}; /* count */
    ...
    while (length > 0)
    {
        ...
        if (list->ob_stat == 3 || list->ob_stat == 4)
        {
            if (xstart > -100 && xstart < 400 && ystart > -100 && ystart < 300)
            {
                ...
                if (list->ob_id == 9 && list->ob_stat == 3)
                    witchflag = TRUE;
            }
            else
                goto loopend;
        }
        if (list->ob_stat == 2 || list->ob_stat == 0)
            goto loopend;
...
int16_t sun_colors[] = {0x000, 0x000, 0x001, 0x002, 0x002, 0x012, 0x112, 0x113, 0x113, 0x214, 0x224,
                        0x225, 0x326, 0x326, 0x337, 0x338, 0x438, 0x439, 0x449, 0x54A, 0x54B, 0x54B,
                        0x55C, 0x65C, 0x65D, 0x66E, 0x76E, 0x76F, 0x86F, 0x97E, 0xA7E, 0xB7E, 0xB8D,
                        0xC8D, 0xD8D, 0xD8D, 0xE9C, 0xE9C, 0xE9B, 0xE9B, 0xFAA, 0xFAA, 0xF99, 0xF98,
                        0xF98, 0xF97, 0xF86, 0xF85, 0xF84, 0xF84, 0xF93, 0xF92, 0x76F};

extern int16_t blackcolors[32];

extern int16_t princess;
...
// TODO: Xark: double check nested else stuff
void day_fade(void)
{
    register int32_t ll;
...
void do_tactic(int32_t i, int32_t tactic)
{
    ...
    {
        if (r)
            set_course(i, hero_x, hero_y, 4);
    }
    else if (tactic == BACKUP)
    {
        if (r)
            set_course(i, hero_x, hero_y, 3);
    }
    else if (tactic == FOLLOW)
    {
        f = leader; /* follow the leader, of course! */
        if (i == f)
            an->tactic = RANDOM; /* can't follow self!! */
        if (r)
            set_course(i, anim_list[f].abs_x, anim_list[f].abs_y + 20, 0);
    }
...
extern int16_t encounter_x, encounter_y;
